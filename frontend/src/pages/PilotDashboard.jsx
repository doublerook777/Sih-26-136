...
export default function PilotDashboard() {
  ...
  return <DashboardLayout role={role} title="Pilot Dashboard" subtitle="Monitor milestone delivery, evidence, governance, and measurable impact.">{loading ? <div className="state-message">Loading pilot data…</div> : error ? <div className="state-message state-error">{error}</div> : !pilot ? <div className="state-message">Pilot not found.</div> : <>
    ...
    <section className="pilot-section"><div className="section-heading"><div><p className="eyebrow">Performance</p><h2>Key performance indicators</h2></div></div>{kpis.length ? <div className="kpi-chart-grid">{kpis.map((kpi) => <KpiChart key={kpi.id} kpi={kpi} pilotId={pilot.id} role={role} onUpdated={(updated) => setKpis((current) => current.map((item) => item.id === updated.id ? updated : item))} />)}</div> : <div className="state-message">No KPIs have been configured.</div>}</section>
